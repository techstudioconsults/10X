...
//       if (data.success) {
//         setUserInfo(data);
//         setImage(data.data.photo || defaultImg);
...
const Profile = () => {
  ...
  const handleImageChange = (event) => {
    ...
    if (file) {
      ...
      setImage(defaultImg); // Set the default image if no file is selected
    }
  };
  ...
  if (data.photo) {
    ...
    formData.append("photo", image); // Use the image state (which may be the default image or the uploaded image)
  }
